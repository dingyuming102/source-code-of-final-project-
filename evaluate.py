...
tsDataloader = DataLoader(tsSet,batch_size=1,shuffle=True,num_workers=1,collate_fn=tsSet.collate_fn)
...
for i, data in enumerate(tsDataloader):
    ...
    if metric == 'nll':
        ...
        if args['use_maneuvers']:
            ...
            f = "pred.txt"
            with open(f, 'a+') as f:
                f.write(str(fut_pred)+'\n')
            '''
            print(fut_pred)
            
            torch.save(fut_pred, "tensor/myTensor.pt")
            break
