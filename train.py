...
#训练中断 重新加载
#net.load_state_dict(torch.load('vlstm_3x13_'+str(1)+'.tar'))
...
if __name__ == '__main__':
    for epoch_num in range(pretrainEpochs+trainEpochs):
        ...
        for i, data in enumerate(trDataloader):
            ...
            if i%100 == 99:
                ...
                f = "tr_cslstm_m(30).txt"
                with open(f, 'a+') as f:
                    f.write("Epoch no:"+str(epoch_num+1)+"| Epoch progress(%):"+format(i/(len(trSet)/batch_size)*100,'0.2f')+ "| Avg train loss:"+format(avg_tr_loss/100,'0.4f')+"| Acc:"+format(avg_lat_acc,'0.4f')+format(avg_lon_acc,'0.4f')+ "| Validation loss prev epoch"+format(prev_val_loss,'0.4f')+ "| ETA(s):"+str(int(eta))+'\n')
        ...
        f = "val_cslstm_m(30).txt"
        with open(f, 'a+') as f:
            f.write('Validation loss :'+format(avg_val_loss/val_batch_count,'0.4f')+"| Val Acc:"+format(avg_val_lat_acc/val_batch_count*100,'0.4f')+format(avg_val_lon_acc/val_batch_count*100,'0.4f')+'\n')
        val_loss.append(avg_val_loss/val_batch_count)
        prev_val_loss = avg_val_loss/val_batch_count

        torch.save(net.state_dict(), 'cslstm_m(30)_3x13_'+str(epoch_num+1)+'.tar')
    ...
    torch.save(net.state_dict(), 'cslstm_m(30).tar')
